...
class LoginView(TokenObtainPairView):
    ...
    throttle_scope = "login"  # DOC 5 §10.3: brute-force protection
...
class RefreshView(TokenRefreshView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # DOC 5 §10.2: with ROTATE_REFRESH_TOKENS, validated_data also
        # carries the rotated refresh — it goes into the cookie, never the
        # response body, so it stays out of JS-readable reach.
...
class LogoutView(APIView):
    """DOC 6 §3: clears the httpOnly refresh cookie. DOC 5 §10.2: also
    explicitly blacklists the refresh token being logged out with, so it
    can't be replayed even though rotation didn't happen. CSRF-protected
    when the ambient cookie is present.
    """
    ...
    def post(self, request):
        ...
        if cookie_refresh:
            _enforce_csrf(request)
        refresh_str = cookie_refresh or (
            request.data.get("refresh") if hasattr(request.data, "get") else None
        )
        if refresh_str:
            try:
...
class RegisterView(APIView):
    ...
    throttle_scope = "register"  # DOC 5 §10.3
...
class AvatarUploadView(APIView):
    """DOC 5 §9, Roadmap Этап 5 п.13: resize/save is a background Celery task
    (apps.accounts.tasks.process_avatar) — this view only validates and
    enqueues. The final avatar URL reaches the client via WS push (event
    `avatar_updated`), not this response, mirroring the existing
    notify_user -> push_notification pattern (apps/notifications/services.py).
    """

    serializer_class = AvatarUploadSerializer

    @extend_schema(request=AvatarUploadSerializer, responses={202: AvatarSerializer})
    def post(self, request):
        file = request.FILES.get("avatar")
        if not file:
            raise ValidationError({"avatar": ["This field is required."]})
        upload_avatar(request.user.profile, file)
