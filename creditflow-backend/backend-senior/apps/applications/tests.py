...
class DocumentPresignedUrlTests(TestCase):
    """DOC 5 §9, Roadmap Этап 5 п.13: private documents → presigned URL with a
    TTL, not a public link. `DocumentSerializer.file` needs no special code —
    it's a plain ModelSerializer FileField, so it already renders through
    `.url`; once storage is S3Boto3Storage with AWS_QUERYSTRING_AUTH=True
    (see config/settings.py), that `.url` call is automatically presigned.
    Verified here against a moto-mocked bucket (no MinIO/Docker required)."""

    def setUp(self):
        self.product = make_product()
        self.user = make_user_with_profile("docs@example.com")
        self.application = CreditApplication.objects.create(
            user=self.user, product=self.product, amount=Decimal("100000.00"), term_months=12,
        )

    def test_document_file_url_is_presigned_when_s3_is_enabled(self):
        import boto3
        ...
        bucket = "creditflow-test-documents"
        with mock_aws():
            ...
            with override_settings(
                ...
                document.file.save("proof.pdf", ContentFile(b"fake-pdf-bytes"), save=True)

                url = DocumentSerializer(document).data["file"]

        self.assertTrue("Expires" in url or "X-Amz-Expires" in url)
