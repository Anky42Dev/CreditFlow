...
class CreditApplication(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            # DOC 5 §14: partial index — the underwriter queue only ever
            # filters MANUAL_REVIEW, so a small, targeted index beats the
            # composite one above for that specific hot path.
            models.Index(
                fields=["submitted_at"],
                name="idx_apps_manual_review_queue",
                condition=Q(status="MANUAL_REVIEW"),
            ),
        ]
