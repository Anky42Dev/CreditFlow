...
class AuditLog(models.Model):
    ...
    (see migration 0002) — see apps.lending.models.Transaction's docstring
    for why `id` is a plain BigIntegerField backed by a DB sequence
    (`audit_logs_id_seq`) instead of an AutoField.
    """

    id = models.BigIntegerField(
        editable=False, db_default=RawSQL("nextval('audit_logs_id_seq')", [])
    )
