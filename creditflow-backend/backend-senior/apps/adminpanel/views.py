...
from apps.applications.services import request_documents
...
class AdminApplicationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_permissions(self):
        ...
        self.required_permission = action_permission.get(
            self.action, "application.view_all"
        )
    ...
    def approve(self, request, pk=None):
        ...
        model = lending_container.approve_application().execute(
            application.id,
            comment=serializer.validated_data["comment"],
            actor=request.user,
            request=request,
        )
        return Response(AdminApplicationDetailSerializer(model).data)
    ...
    def reject(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        model = lending_container.reject_application().execute(
            application.id,
