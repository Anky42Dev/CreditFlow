...
from .push import push_avatar_updated, push_status
...
@override_settings(CHANNEL_LAYERS=TEST_CHANNEL_LAYERS)
class PushAvatarUpdatedDeliveryTests(TransactionTestCase):
    """DOC 5 §9, Roadmap Этап 5 п.13: apps.accounts.tasks.process_avatar pushes
    the final avatar URL over WS once the background resize completes."""

    def setUp(self):
        self.user = User.objects.create_user(email="avatar-ws@example.com", password="pw12345")

    async def test_connected_client_receives_avatar_updated_event(self):
        token = str(AccessToken.for_user(self.user))
        communicator = WebsocketCommunicator(application, f"/ws/notifications/?token={token}")
        connected, _ = await communicator.connect()
        self.assertTrue(connected)

        await sync_to_async(push_avatar_updated, thread_sensitive=False)(
            self.user.id, "https://cdn.example.com/avatars/1.jpg"
        )

        payload = await communicator.receive_json_from()
        self.assertEqual(payload["event"], "avatar_updated")
        self.assertEqual(payload["avatar"], "https://cdn.example.com/avatars/1.jpg")
